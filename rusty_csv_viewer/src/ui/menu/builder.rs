...
use crate::ui::{AppState, Component, MainWindow};
...
pub enum MenuBuildType
{
    ...
    MenuItem(String, bool,
             Box<dyn Fn(&MainWindow, &mut AppState, &nwg::Event, &nwg::EventData) -> BoxedResult<()> + 'static>),
    /// MenuSeparator
    MenuSeparator
}
...
impl MenuBuilder
{
    ...
    pub fn build(&mut self, parent: &nwg::ControlHandle) -> NwgCompResult  {
        ...
        // Call the internal build function
